...
- 领取规则：状态为 `pending` 且未持有有效（未过期）租约的指令中，取 **ID 最小**的一条。
...
- 只有**未过期且属于当前代次**的令牌可以结算。裁决顺序（行锁内原子完成）：
  1. 指令已在终态 → `409 already_settled`（重复结算/覆盖一律拒绝）；
...
- 未结算时 `settlement` 为 `null`。未知编号 `404 unknown_command`。
...
- `commands`：指令本体、当前 `status`（`pending|delivered|failed`）、当前
  `lease_generation`、当前令牌与到期时间。
...
（行锁 + 终态裁决 + `settlements` 插入 + 状态翻转），因此任何崩溃/重启都不会留下
半完成的裁决。
